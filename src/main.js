...
const getDataJSON = async (nodes) => {
  ...
  let results = await application.createRenditions(
    nodes.map((node, i) => {
      return {
        node: node,
        outputFile: files[i],
...
async function exportCurrentArtboard(selection, documentRoot) {
  ...
  await saveNodes(
    items,
    documentRoot.name || items[0].name || "inker8-exports"
  );
}

async function testPrompt(selection, documentRoot) {
  // console.log('application.appLanguage', application.appLanguage)
  prompt("Test");
}
...
async function exportAllArtboards(selection, documentRoot) {
  ...
  documentRoot.children.forEach((child, i) => {
    items[i] = child;
  });
